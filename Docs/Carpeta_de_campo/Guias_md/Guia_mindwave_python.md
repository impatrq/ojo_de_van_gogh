# Programas para  probar Mindwave

## Programa receptor de Parpadeo 

Este programa consiste en detectar 2 parpadeos y mandar un 1 por serial. Esto se hace a través de varias funciones que comprueban el estado de los registros y datos que recibe para percibir cuando haya dos parpadeos y mandar un 1 al Arduino que luego procesa esa información y prende el led del pin 13 que viene incorporado.
...
## Programa de Arduino receptor del Código de Python

```c++
const int pinLED = 13; // definis el led del arduino
...
void loop()
{
  if (Serial.available()>0) //si te llega info
  {
   	char option = Serial.read();
   		if (option == '1')	// si llega uno se prende el led
   		{
		digitalWrite(pinLED, HIGH);
     	delay(1000);
		}

		else	//si llega 0 se apaga
		{
